...
def eval_fn(
    ...
    all_out_list = []
    all_prompts = []
    all_rewards_cur = {'zb_gemini':[], 'avg':[]}
    # all_rewards_cur = {'zb_gemini':[], 'sam_iou':[], 'avg':[]}
    # all_rewards_cur = {'gemini_aes':[], 'zb_gemini':[], 'sam_iou':[], 'avg':[]}
    ...
    for test_batch in tqdm(
        ...
        # 收集结果
        all_rewards_keys = rewards.keys()
        for key in all_rewards_keys:
            if key not in rewards:
                pass
            else:
                all_rewards_cur[key].extend(rewards[key])
    ...
    if is_main_process(rank):
        ...
        dst_folder = os.path.join('/mmu-vcg/zb08/outputs/texture_edit/aa_debug', now_str.replace(' ', '_').replace(':', '-')+f'_{global_step}_eval')
        ...
        for idx, item in enumerate(zip(all_prompts, all_ref_list, all_out_list)) :
            ...
            scores = {key: all_rewards_cur[key][idx] for key in all_rewards_cur}
            scores_str = '_'.join([f"{k}-{v:.2f}" for k, v in scores.items()])

            prompt_short = prompt[:50].replace(' ', '_').replace('/', '_')
            ref_image.save(os.path.join(orig_f, f"{prompt_short}_{idx}_{scores_str}.png"))
            edited_img_pil = Image.fromarray(
                (edited_image.cpu().float().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
            )
            edited_img_pil.save(os.path.join(edited_f, f"{prompt_short}_{idx}_{scores_str}.png"))
...
def save_ckpt(
    ...
    if is_main_process(rank):
        ...
        save_root = os.path.join(save_dir, "checkpoints_"+f"{unique_id}", f"checkpoint-{global_step}")
...
def main(_):
    ...
    # test_dataset = PromptImageDataset(config.dataset, config.resolution, "debug")
    ...
    train_sampler = DistributedKRepeatSampler(
        ...
        rank=rank,
        seed=config.seed,
        banned_prompts=banned_prompts_list,  # 初始为空
    )
    ...
    # 1_4  更新 reward function
    import reward_edge_loss
    reward_fn = reward_edge_loss.multi_score(device, config.reward_fn)
    eval_reward_fn = reward_edge_loss.multi_score(device, config.reward_fn)
    ...
    # reward_fn = getattr(flow_grpo.zb_reward_test_sam, "multi_score")(
    #     device, config.reward_fn
    # )  # Pass device
    # eval_reward_fn = getattr(flow_grpo.zb_reward_test_sam, "multi_score")(
    #     device, config.reward_fn
    # ) 
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        if epoch > 0 and is_main_process(rank):
            save_p = '/mmu-vcg/zb08/outputs/texture_edit/aaa_utils/new'
            cur_names = f"banned_prompts_epoch_{epoch}.json"
            pppp = os.path.join(save_p, cur_names)
            with open(pppp, 'w') as f:
                json.dump(list(stat_tracker.banned_prompts), f, indent=4)
        ...
        for i in tqdm(
            ...
            # # ref imags 也要存
            # dst_folder = os.path.join('/mmu-vcg/zb08/outputs/texture_edit/aa_debug2', f'_{global_step}_train_rollout_source')
            # os.makedirs(dst_folder, exist_ok=True)
            # for idx, ref_image in enumerate(ref_images) :
            #     prompt_short = prompts[idx][:80].replace(' ', '_').replace('/', '_')
            #     ref_image.save(os.path.join(dst_folder, f"sample_{global_step}_{i}_{idx}_{prompt_short}.png"))
            

            # dst_folder = os.path.join('/mmu-vcg/zb08/outputs/texture_edit/aa_debug2', f'_{global_step}_train_rollout_edit')
            # os.makedirs(dst_folder, exist_ok=True)

            # scores = rewards_future.result()[0]['zb_gemini']
            # t2 = images.cpu()
            ...
            #     pil.save(os.path.join(dst_folder, f"flow20s_sample_{global_step}_{i}_{idx}_{cur_score}.png"))
    ...
    if is_main_process(rank):
        # 保存  banned_prompts
        # save_p = '/mmu-vcg/zb08/outputs/texture_edit/aaa_utils/banned_prompts.json'
        # with open(save_p, 'w') as f:
        #     json.dump(list(train_sampler.banned_prompts), f, indent=4)
