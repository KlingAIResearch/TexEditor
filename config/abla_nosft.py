...
def _get_config(base_model="qwen_image_edit", n_gpus=1, gradient_step_per_epoch=1, reward_fn={}, name=""):
    ...
    config.dataset = "/mmu-vcg/zb08/outputs/texture_edit/train_data/rl_data/wash_rl_data_1_8"
    # config.dataset = "/mmu-vcg/zb08/outputs/texture_edit/train_data/rl_data/maybe_v2_final_texture"
    # config.dataset = "/mmu-vcg/zb08/outputs/texture_edit/train_data/rl_data/maybe_final_texture"
    # config.dataset = "/mmu-vcg/zb08/outputs/texture_edit/train_data/rl_data/temp_texture"
    # config.dataset = "/mmu-vcg/zb08/outputs/texture_edit/train_data/rl_data/temp"
    # config.dataset = "/mmu-vcg/zb08/outputs/texture_edit/train_data/blender"
    
    config.pretrained.model = "/mmu-vcg/zhongweizhi/Pretrained_ckp/ckp_Qwen-Image-Edit-2509"
    # config.pretrained.model = "Qwen/Qwen-Image-Edit-2509"
    config.sample.num_steps = 18
    ...
    # config.sample.solver = "dance"
    # config.sample.solver = "dpm2"
    return config

def qwen_mllm_reward():
    reward_fn = {"sam_iou": 0.45, "zb_gemini": 0.55}
    # reward_fn = {"sam_iou": 0.25, "zb_gemini": 0.75}
    # reward_fn = {"sam_iou": 0.45, "zb_gemini": 0.45, "gemini_aes":0.1}
    config = _get_config(
        base_model="qwen_image_edit",
        n_gpus=8,
        gradient_step_per_epoch=1,
        reward_fn=reward_fn,
        name="zb_sft_base_bigbs_Coco_normal_kl_allwash_abla_noedgeloss",
    )
