...
def remove_small_components(
    mask: np.ndarray,
    min_area_ratio: float = 0.01,  # 最小面积占比（1%）
):
    """
    mask: shape (H, W)
    """
...
def shape_loss (device=None):
    ...
    def evaluate_sam_blocking(
        ...
        img_t = transforms.ToTensor()(img).unsqueeze(0).to(ddd)
        with torch.no_grad():
            multi_outputs, outputs, _, _= model(img_t)
        img_edge = post_process_edge(multi_outputs)

        ref_img_t = transforms.ToTensor()(ref_img).unsqueeze(0).to(ddd)
        with torch.no_grad():
        ...
        # 这里是 在GPu 上的torch.bool的tensor 其实后面用起来相当麻烦

        import datetime
        unique_id = datetime.datetime.now().strftime("%Y.%m.%d_%H.%M")

        # 可视化保存。可以注释掉
        vis_dir = '/mmu-vcg/zb08/outputs/texture_edit/aa_debug/aaa_sam_reward_vis_debug'
        real_vis_dir = os.path.join(vis_dir, unique_id)
        ...
        # mask_1_cleaned = remove_small_components(mask11, min_area_ratio=0.015)
        # mask_2_cleaned = remove_small_components(mask22, min_area_ratio=0.015)

        # m_iou = mask_iou_simple(mask_1_cleaned, mask_2_cleaned)
        ...
        # mask_ref_c = alpha_plus(ref_img, mask_1_cleaned, default_color=(255,0,0))
        # mask_img_c = alpha_plus(img, mask_2_cleaned, default_color=(0,0,255))

        img_edge = (np.clip(img_edge*255,0,255)).astype(np.uint8)
        # img_edge变成3通道 
        img_edge_3ch = np.stack([img_edge, img_edge, img_edge], axis=-1)

        ref_img_edge = (np.clip(ref_img_edge*255,0,255)).astype(np.uint8)
        ref_img_edge_3ch = np.stack([ref_img_edge, ref_img_edge, ref_img_edge], axis=-1)


        # m_iou = edge_iou(img_edge, ref_img_edge)
        m_iou = edge_iou_tolerant(img_edge, ref_img_edge)
        ...
        # combined = np.concatenate([ mask_ref, img_edge, mask_img, ref_img_edge, mask_vis], axis=1)
        combined = np.concatenate([ref_img, mask_ref, img_edge_3ch, img, mask_img, ref_img_edge_3ch, mask_vis], axis=1)
        save_path = os.path.join(
            real_vis_dir,
            # "ssim_edge_"+ str(score) + "_sam_iou_"
            "ssim_edge_"+ str(score) + "_sam_iou_" + str(round(m_iou,4)) + ".png"
        )
    ...
    def evaluate_sam_threaded(
        ...
        for i in range(len(prompts)):
            # try:
            res = evaluate_sam_blocking(ref_imgs[i],images[i],prompts[i],metadata[i],processor,model)
            results[i] = res
            # except Exception as e:
            #     print(f"❌ Unexpected thread error: {e}")
            #     results[i] = 0
