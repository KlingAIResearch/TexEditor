...
class AestheticScorer(torch.nn.Module):
    def __init__(self, dtype, device):
        super().__init__()
        clip_dir = '/mmu-vcg/zb08/CKPTS/clip'
        self.clip = CLIPModel.from_pretrained(clip_dir).to(device)
        self.processor = CLIPProcessor.from_pretrained(clip_dir)
        self.mlp = MLP().to(device)
        state_dict = torch.load(os.path.join("/mmu-vcg/zb08/CKPTS/weights", "sac+logos+ava1-l14-linearMSE.pth"), map_location="cpu")
