...
LOG_PREFIX="score_filter_${TIMESTAMP}"

for i in $(seq 0 $((NUM_INSTANCES - 1)))
do
    echo "启动第 $i 个"
    nohup env CUDA_VISIBLE_DEVICES=$i \
    python /mmu-vcg/zb08/codes/UniWorld-main/UniWorld-V2/zb_scripts/scores/filter_texture.py --num_splits $NUM_INSTANCES --split_index $i \
