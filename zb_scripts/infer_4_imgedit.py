...
class PromptImageDataset(Dataset):
    def __init__(self, dataset, resolution=1024, split="train"):
        ...
        self.img_dir = "/mmu-vcg/liuzheng/liuz/dataset/benckmark/ImageEdit_Benchmark/singleturn"
        with open(self.file_path, "r", encoding="utf-8") as f:
            self.metadatas = [json.loads(line) for line in f]
            self.prompts = [item["prompt"] for item in self.metadatas]
            self.ids = [item["id"] for item in self.metadatas]
            self.edit_types = [item["edit_type"] for item in self.metadatas]
            self.save_ids = [item["idx"] for item in self.metadatas]
    ...
    def __getitem__(self, idx):
        ...
        cur_id = self.metadatas[idx]["id"]
        image_path = os.path.join(self.img_dir, cur_id)
        ...
        item["image"] = image
        item["edit_type"] = self.edit_types[idx]
        item['save_id'] = self.save_ids[idx]
        return item
    ...
    def collate_fn(examples):
        ...
        edit_types = [example["edit_type"] for example in examples]
        save_ids = [example["save_id"] for example in examples]
        return prompts, metadatas, images, prompt_with_image_paths, ids, edit_types,save_ids
...
def eval_fn(
    ...
    for test_batch in tqdm(
        ...
        prompts, prompt_metadata, ref_images, prompt_with_image_paths, ids, edit_types, save_ids = test_batch
        ...
        with torch_autocast(
            ...
            all_ids.extend(ids)
            all_edit_types.extend(edit_types)
            all_save_ids.extend(save_ids)
    ...
    # 打印 rank 和 all_save_ids
    print("rank:", rank, "all_save_ids:", all_save_ids)
    # print()

    for idx, item in enumerate(zip(all_prompts, all_ref_list, all_out_list, all_ids, all_edit_types, all_save_ids)) :
        prompt, ref_image, edited_image, cur_id, edit_type, save_id = item

        save_dir = os.path.join(dst_folder,'edit')
        os.makedirs(save_dir, exist_ok=True)


        real_img_name = str(save_id) + '.png'


        prompt_short = prompt[:80].replace(' ', '_').replace('/', '_')
        ref_image.save(os.path.join(orig_f, real_img_name))
        edited_img_pil = Image.fromarray(
            (edited_image.cpu().float().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
        )
        edited_img_pil.save(os.path.join(save_dir, real_img_name))
...
def main(_):
    ...
    lora_path = '/mmu-vcg/zb08/codes/UniWorld-main/UniWorld-V2/logs/nft/qwen_image_edit/zb_sft_base_bigbs_Coco_normal_kl_allwash_resume_112/checkpoints_2026.01.13_05.56.04/checkpoint-72/lora'
    ...
    # test_dataset = PromptImageDataset(config.dataset, config.resolution, "temp")
    test_dataset = PromptImageDataset(config.dataset, config.resolution, "single_turn_new")
    # test_dataset = PromptImageDataset(config.dataset, config.resolution, "more_index")
