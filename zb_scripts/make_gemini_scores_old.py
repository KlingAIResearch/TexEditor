...
if __name__ == "__main__":
    ...
    dst_foldr = '/mmu-vcg/zb08/outputs/texture_edit/final_old_scores/origin_dpm20_texture_v3'

    os.makedirs(dst_foldr, exist_ok=True)

    # 输入图像与指令都是一样的
    in_p = '/mmu-vcg/zb08/outputs/texture_edit/final_scores/orgin_dpm20_final_v2_texture_edge_5050.jsonl'

    dst_img_p = '/mmu-vcg/zb08/outputs/texture_edit/sft_base/2026.01.04_11.26_0_eval_origin_dpm20-tsts_real/edited'
    ...
    for item in tqdm(sub_data):  # 
        ...
        if (not os.path.exists(img_path)) or (not os.path.exists(d_img_path)):
            print(f"❌ Image not found: {img_id_name}")
            continue
        ...
        Your task is to evaluate how well the texture editing instruction has been followed. The shape and structure should remain consistent unless specified otherwise. And extra object should not be added. Please be strict!
        First, provide a concise justification in no more than 25 words explaining your judgment.
        Then, assign a score from 0 to 10 reflecting the completion quality of the instruction.
